    ...
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-emerald-950/30">
      ...
      <div className="flex-1 container mx-auto py-8 px-6 flex flex-col items-center">
        <div className="mb-12 text-center space-y-6">
          <h1 className="text-3xl font-bold text-emerald-600">Best Practices</h1>
          ...
          <div className="inline-flex rounded-lg p-0.5 bg-zinc-900/80 border border-zinc-800/80">
            <button
              ...
              className={`px-6 py-2 rounded-md text-sm font-medium transition-all duration-200 ${
                !showMixed 
                ? 'bg-emerald-600/20 text-emerald-400 shadow-lg border border-emerald-500/20' 
                : 'text-zinc-400 hover:text-emerald-400/80'
              }`}
            >
              Tools
            </button>
            <button
              ...
              className={`px-6 py-2 rounded-md text-sm font-medium transition-all duration-200 ${
                showMixed 
                ? 'bg-gradient-to-r from-emerald-600/20 to-pink-600/20 text-transparent bg-clip-text border border-emerald-500/10 border-r-pink-500/10 shadow-lg' 
                : 'text-zinc-400 hover:text-white'
              }`}
            >
              <span className={showMixed ? 'bg-gradient-to-r from-emerald-400 to-pink-400 text-transparent bg-clip-text' : ''}>
                Mixed
              </span>
            </button>
          </div>
        ...
        <div className={`grid ${showMixed ? 'grid-cols-1 max-w-[600px]' : 'grid-cols-4'} gap-6 mb-12 max-w-[1400px] w-full mx-auto`}>
          {(showMixed ? MIXED_TOOLS : TOOLS).map((tool) => (
            <Link 
              ...
              className="group"
            >
              <Card className="relative bg-zinc-900 border-zinc-800/50 hover:border-emerald-800/50 transition-all duration-300 h-[280px] overflow-hidden rounded-xl">
                {/* Background Pattern */}
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_100%_100%,_rgba(16,185,129,0.1)_0,_transparent_50%)] opacity-70"></div>
                  ...
                  <div className="flex items-start gap-4">
                    {'primaryColor' in tool ? (
                      <div className="flex items-center -space-x-3">
                        <div className="w-12 h-12 rounded-lg bg-emerald-950/50 border border-emerald-800/30 flex items-center justify-center z-10">
                          <CursorIcon className="w-6 h-6 text-emerald-500" />
                        </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <h2 className={`text-lg font-semibold truncate transition-colors ${
                        'primaryColor' in tool 
                          ? 'bg-gradient-to-r from-emerald-400 to-pink-400 text-transparent bg-clip-text'
                          : tool.id === 'cursor' 
